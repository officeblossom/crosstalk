...
type Location = { id: number; name: string; slug: string; venue: string; lastCount: number };
type EventItem = { id: number; slug: string; edition: number; eventDate: string; venue: string; locationName: string; isOpen: boolean; registrationCount: number };
...
export default function AdminPage() {
  ...
  async function createEvent(e: FormEvent<HTMLFormElement>) {
    ...
    await action({ action: "createEvent", locationId: location.id, edition: data.get("edition"), eventDate: data.get("date"), venue: data.get("venue") });
    e.currentTarget.reset();
  }
  async function createLocation(e: FormEvent<HTMLFormElement>) {
    e.preventDefault(); const data = new FormData(e.currentTarget);
    await action({ action: "createLocation", name: data.get("name"), slug: data.get("slug"), venue: data.get("venue"), lastCount: data.get("lastCount") });
    e.currentTarget.reset();
  }
  ...
  return <main className="admin-shell">
    ...
    <div className="admin-content">
      ...
      <section className="admin-panel" id="locations"><div className="panel-title"><span>03</span><div><p>LOCATIONS</p><h2>地域・会場の追加</h2></div></div>
        <div className="location-chips">{locations.map((x) => <div key={x.id}><strong>{x.name}</strong><span>{x.venue}</span><small>開催済み {x.lastCount}回</small></div>)}</div>
        <form className="create-form location-form" onSubmit={createLocation}><label>地域名<input name="name" placeholder="例：Kokura" required/></label><label>URL名（半角英数）<input name="slug" placeholder="kokura" pattern="[a-zA-Z0-9-]+" required/></label><label>開催済み回数<input name="lastCount" type="number" min="0" defaultValue="0"/></label><label className="wide">デフォルト会場<input name="venue" placeholder="会場名" required/></label><button className="admin-primary">地域を追加する ＋</button></form>
      </section>
